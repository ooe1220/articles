# 初めに
筆者はゲームの表示原理に興味があります。<br>
いきなり3Dや高度な処理を扱うのは難しい為、まずは静止画を用いてアフィン変換や射影変換の実験を行い、段階的に理解を深めていきたいと考えています。<br>

その第一歩として、本記事では画像の保存形式であるBMPの仕組みを確認します。<br>

尚、筆者自身も調査と実験を進めながら理解している段階の為、内容に誤り等がありましたらご指摘いただけますと幸いです。<br>
...
※4bit境界に合わせる式は筆者が以下の記事で解説しています。<br>
[4バイト境界調整式 (アライメント)](https://qiita.com/earthen94/items/066445bb02f3cdd0ae8d)<br>

# BITMAPの行は下から上へ読む
BITMAPの仕様で`biHeight`が正の場合は下の行から上に格納されます(説明が下手なので仕様は他のサイトで確認して下さい)。<br>
...
バイナリを見てみると仕様通り最終行が一番上に格納されています。

即ち、例えば以下の様な画像があった場合に<br>
```
赤青紫藍
白黄橙茶
桃黒緑灰
```
バイナリ上は以下の様な配列で格納されます。<br>
```
桃黒緑灰
白黄橙茶
赤青紫藍
