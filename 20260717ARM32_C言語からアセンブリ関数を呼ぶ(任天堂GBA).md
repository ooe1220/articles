...
`main.c`の中に色々なものを詰め込み過ぎていたので、画面表示に関する処理は全て`gfx.c`にまとめました。
`main.c`及び`gfx.c`以外のソースは一切変更していません。
...
arm-none-eabi-ld tmp/header.o tmp/add.o tmp/main.o tmp/font.o tmp/gfx.o -T link.ld -o tmp/out.elf
arm-none-eabi-objcopy -O binary tmp/out.elf tmp/out.gba
```
<details>
<summary>link.ld</summary>
...
void main(void)
{
    ...
    println("");
    gfx_set_text_color(COLOR_RED);  // 赤文字
    println("DEBUG");
...
<img width="4096" height="3072" alt="ea298208dd55f8" src="https://github.com/user-attachments/assets/e957137e-c307-481a-9a6b-b64537651d49" />
