...
20本のアドレス線しかないため、21bit目に相当するA20を利用できず、アドレスが`1MiB(0x100000)`以上になると折り返して0x0に戻ってしまいます。
32bitアドレス空間を持つ386では最大4GiBの物理アドレスを扱えるようになりましたが、8086時代に1MBの折り返しを利用したプログラムもあったらしく、互換性の為に`A20`という仕組みが作られました。
CPU通電直後では`A20`が無効になっており、明示的にA20を有効化する必要があります。

今回はこの`A20`の開放について実験をします。
...
`0x000500`には`0xAA`の値、`0x100500`には`0x55`の値を入れました。
