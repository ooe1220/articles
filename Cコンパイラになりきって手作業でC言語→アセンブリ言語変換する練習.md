...
```
    ...
    mov dword [ebp-4],eax
```
のようになると予想していましたが、最適化無しの場合はコンパイラは各変数をレジスタに入れてから、レジスタ同士を加算するコードを生成しました。
```
	mov	edx, DWORD PTR [ebp+8]
	mov	eax, DWORD PTR [ebp+12]
	add	eax, edx
```
最適化ありの場合は筆者の書き方と殆ど同じになりました。
※add関数は残っていますが結果としてmainからは呼び出していません。
※push ebpが無く直接esp基準で呼び出しているので添字は全て-4となります。
```
	mov	eax, DWORD PTR [esp+8]
	add	eax, DWORD PTR [esp+4]
```
