...
これはXORの動作を理解するには分かりやすい回路です。

しかし、今回のCPUではNANDのみを使って回路を構成するため、この回路をNANDだけで置き換えるとNANDの数が増えてしまいます。

そこで、今回はNAND 4個だけで構成できる回路を使用します。

※今回の様に、真理値表の $y=1$ の行を全て拾って式にする方法を、積和標準形といいます。
